...
def print_valve(print_height_abs, theta=0, stem_print_speed = default_print_speed, flow_connection_x = 3, control_connection_y = 3, x_mirror=False, y_mirror=False, flow_inlet = True, control_inlet = True, control_stem_corner = True):
    ...
    x_mirror_mult = (-1 if x_mirror else 1)
    y_mirror_mult = (-1 if y_mirror else 1)
    ...
    def print_inlet(_theta=theta):
        ...
        move_y(y_mirror_mult*inlet_stem_length, _theta)
        PrintingGlobals.g.feed(inlet_print_speed)
        move_y(y_mirror_mult*inlet_needle_length, _theta)
    ...
    was_relative = PrintingGlobals.g.is_relative_mode()
    if (was_relative==False):
...
#    travel_mode() #assume we're already in travel mode
    move_xy(x_distance=x_mirror_mult*(-control_pad_length/2.0), y_distance=y_mirror_mult*(-control_pad_width/2.0),theta=theta)
    ...
    for meander in range(n_meanders):
        move_y(y_mirror_mult*(y_sign*control_pad_width), theta)
        move_x(x_mirror_mult*(control_pad_meander_separation), theta)
        y_sign = -1*y_sign
    move_y(y_mirror_mult*(y_sign*control_pad_width/2), theta)
    
    #print the stem from the bottom control pad
    PrintingGlobals.g.feed(stem_print_speed)
    move_x(x_mirror_mult*(control_pad_stem_length), theta) #go to the stem to the right before corner(?) to inlet
    
    control_stem_and_inlet_recover_x=control_pad_stem_length
    control_stem_and_inlet_recover_y=0
    control_stem_alignement_distance=0
    if (control_inlet):
        if (control_stem_corner):
            ...
            move_y(y_mirror_mult*(control_stem_alignement_distance), theta)
            print_inlet(_theta=theta ) #print the control inlet up
            control_stem_and_inlet_recover_y += control_stem_alignement_distance+inlet_total_length
        #no corner. Inlet in X (must take in to account the mirror
        else:
            inlet_theta = theta - np.pi/2
            if (x_mirror):
                print "X Mirror inlet!"
                inlet_theta = inlet_theta + np.pi
            print_inlet(_theta =  inlet_theta)
            control_stem_and_inlet_recover_x += inlet_total_length


            
    #travel to the back (-y) interconnect point of flow lines from the tip of the inlet
    travel_mode(whipe_angle=theta+np.pi/2)
    move_xy(x_distance = x_mirror_mult*(-control_stem_and_inlet_recover_x - control_pad_length/2.0), y_distance = y_mirror_mult*(-control_stem_and_inlet_recover_y - flow_stem_length - 0.5*control_pad_width), theta=theta)
    print_mode(print_height_abs=print_height_abs+pad_z_separation)
        
    #bottom flow line
    PrintingGlobals.g.feed(stem_print_speed)
    move_y(y_mirror_mult*(flow_stem_length+0.5*(control_pad_width-flow_pad_length)), theta)
    
    #flow pad
    n_meanders = int(flow_pad_length/flow_pad_meander_separation)
    x_sign = -1
    PrintingGlobals.g.feed(flow_pad_print_speed)
    move_x(distance=x_mirror_mult*(flow_pad_width/2.0), theta=theta)
    for meander in range(n_meanders-1):
        move_y(y_mirror_mult*(flow_pad_meander_separation),theta)
        move_x(x_mirror_mult*(x_sign * flow_pad_width),theta)
        x_sign = -1*x_sign
    move_y(y_mirror_mult*(flow_pad_meander_separation),theta)    
    move_x(x_mirror_mult*(x_sign*(flow_pad_width/2.0)), theta)
    
    #top flow line
    PrintingGlobals.g.feed(stem_print_speed)
    move_y(y_mirror_mult*(flow_stem_length+(control_pad_width-flow_pad_length)/2.0), theta)
    ...
    move_xy(x_distance=x_mirror_mult*(junction_dist+0.5*control_pad_length), y_distance = y_mirror_mult*((-inlet_total_length if flow_inlet else 0)-flow_stem_length-(control_pad_width-flow_pad_length)/2.0-flow_pad_length/2.0), theta=theta)
    ...
    PrintingGlobals.g.feed(stem_print_speed)
    move_x(x_mirror_mult*(-junction_dist), theta)
...
#    move_x(-0.5*(control_pad_width-flow_pad_length), theta=theta)
    ...
    y_sign = 1
    n_meanders = int(control_pad_length/control_pad_meander_separation)
    move_y(y_mirror_mult*(-control_pad_width/2.0), theta)
    move_x(x_mirror_mult*(-control_pad_meander_separation), theta)
    for meander in range(n_meanders-1):
        move_y(y_mirror_mult*(y_sign*control_pad_width), theta)
        move_x(x_mirror_mult*(-control_pad_meander_separation), theta)
